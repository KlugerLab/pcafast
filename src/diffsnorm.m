...
%
%   output:
%   snorm -- an estimate of the spectral norm of A-USV' or c(A) - USV' 
%           (the estimate fails to be accurate with exponentially low probability
...
if(m >= n)
    ...
    %
    x = randn(n,1) + (~isreal(A) || ~isreal(U) || ~isreal(S) || ~isreal(V)) * 1i*randn(n,1);
    x = x/norm(x);
...
if(m < n)
    ...
    y = randn(1,m) + (~isreal(A) || ~isreal(U) || ~isreal(S) || ~isreal(V)) * 1i*randn(1,m);
    y = y/norm(y);
